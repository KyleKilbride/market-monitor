...
import { Source, FeedItem, KeywordRule } from '../types/sources.js';
...
const failingSourcesCooldown = new Map<string, number>();
...
async function fetchFeed(parser: Parser, source: Source, maxRetries = 3): Promise<any> {
...
export async function startMonitoring(options: MonitorOptions) {
  ...
  const processedItems = new Map<string, boolean>();
    ...
    for (const source of config.sources) {
      try {
        if (source.type === 'rss' && source.url) {
          ...
          let keywordsToUse: (string | KeywordRule)[] = [];
...
function handleNewItem(item: FeedItem, silent: boolean) {
  if (!silent) {
    try {
