...
function formatKeyword(k: string | KeywordRule, i: number): string {
  if (typeof k === 'string') {
...
export function setupKeywordCommands(program: Command) {
  ...
  keywords
    ...
    .action(async () => {
      try {
        ...
        config.sources[0].keywords.forEach((k: string | KeywordRule, i: number) => {
          if (typeof k === 'string') {
  ...
  keywords
    ...
    .action(async () => {
      try {
        const config = await loadConfig();
        const choices = config.sources[0].keywords.map((k: string | KeywordRule, i: number) => ({
        ...
        const indices = selected.sort((a: number, b: number) => b - a);
        const removed = [];
        ...
        removed.reverse().forEach((k: string | KeywordRule) => {
          if (typeof k === 'string') {
